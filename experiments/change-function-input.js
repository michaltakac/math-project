...
// Range of function input variable X -----
const fn = mathbox.select("#main-function");

const functionXMinSlider = document.getElementById("function-x-min-slider");
const functionXMinValue = document.getElementById("function-x-min-value");
functionXMinValue.innerHTML = -functionXMinSlider.value / 100;

functionXMinSlider.oninput = function() {
  state.rangeXMin = -this.value / 100;
  functionXMinValue.innerHTML = state.rangeXMin;
  fn.set("rangeX", [state.rangeXMin, state.rangeXMax]);
  fn.set("expr", function(emit, x, y, i, j) {
    const func = Math.pow(x, 2) - Math.pow(y, 2);
    emit(x, func, y);
  });
};

const functionXMaxSlider = document.getElementById("function-x-max-slider");
const functionXMaxValue = document.getElementById("function-x-max-value");
functionXMaxValue.innerHTML = functionXMaxSlider.value / 100;

functionXMaxSlider.oninput = function() {
  state.rangeXMax = this.value / 100;
  functionXMaxValue.innerHTML = state.rangeXMax;
  fn.set("rangeX", [state.rangeXMin, state.rangeXMax]);
  fn.set("expr", function(emit, x, y, i, j) {
    const func = Math.pow(x, 2) - Math.pow(y, 2);
    emit(x, func, y);
...
// Range of function input variable Y ----

const functionYMinSlider = document.getElementById("function-y-min-slider");
const functionYMinValue = document.getElementById("function-y-min-value");
functionYMinValue.innerHTML = -functionYMinSlider.value / 100;

functionYMinSlider.oninput = function() {
  state.rangeYMin = -this.value / 100;
  functionYMinValue.innerHTML = state.rangeYMin;
  fn.set("rangeY", [state.rangeYMin, state.rangeYMax]);
  fn.set("expr", function(emit, x, y, i, j) {
    const func = Math.pow(x, 2) - Math.pow(y, 2);
    emit(x, func, y);
  });
...
const functionYMaxSlider = document.getElementById("function-y-max-slider");
const functionYMaxValue = document.getElementById("function-y-max-value");
functionYMaxValue.innerHTML = functionYMaxSlider.value / 100;

functionYMaxSlider.oninput = function() {
  state.rangeYMax = this.value / 100;
  functionYMaxValue.innerHTML = state.rangeYMax;
  fn.set("rangeY", [state.rangeYMin, state.rangeYMax]);
  fn.set("expr", function(emit, x, y, i, j) {
    const func = Math.pow(x, 2) - Math.pow(y, 2);
    emit(x, func, y);
